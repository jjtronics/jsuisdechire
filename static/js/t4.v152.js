(function(){
  ...
  function onOri(e){
    src.do=true;
    // Orientation contributes a small normalized movement proxy (unitless)
    const g=Math.hypot(e.alpha||0,e.beta||0,e.gamma||0)/180;
    pushVal(g*0.05); // small weight to smooth series
  }
  ...
  function sensorsSuccess(){
    ...
    const s = Math.max(0, Math.min(100, Math.round(100*(1 - (std-0.01)/(0.20-0.01)) )));
