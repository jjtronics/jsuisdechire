(function(){
  ...
  function onDev(e){ src.dm=true; const a=e.accelerationIncludingGravity||e.acceleration||{x:0,y:0,z:0}; const g=Math.hypot(a.x||0,a.y||0,a.z||0); pushVal(g); }
  function onOri(e){ src.do=true; const g=Math.hypot(e.alpha||0,e.beta||0,e.gamma||0)/180; pushVal(g); }
  ...
  function sensorsSuccess(){
    ...
    localStorage.setItem('jsd:bal', JSON.stringify({ mode:'sensors', duration_ms:8000, std_g:std, score:Math.max(0,Math.min(100,(1-(std-0.03)/(0.18-0.03))*100)) }));
  ...
  async function start(){
    ...
    watchdog=setInterval(()=>{
      ...
      // IMPORTANT: ne bascule que si AUCUN event après 1.5s
    ...
    endTimer=setTimeout(()=>{
      ...
      // Succès capteurs dès qu'il y a AU MOINS 1 event
