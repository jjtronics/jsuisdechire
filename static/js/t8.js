(function () {
  ...
  const layout = () => { const width = Math.max(280, stage.clientWidth || 360); const height = Math.max(280, stage.clientHeight || 352); state.cup.width = clamp(width * .16, 84, 120); state.cup.y = cup.offsetTop + 5; state.cup.x = clamp(state.cup.x || width / 2 - state.cup.width / 2, 12, width - state.cup.width - 12); ball.style.left = '50%'; ball.style.top = `${height - 83}px`; updateCup(); };
  const setAim = (target) => { const ballRect = ball.getBoundingClientRect(); const stageRect = stage.getBoundingClientRect(); const start = { x: ballRect.left + ballRect.width / 2 - stageRect.left, y: ballRect.top + ballRect.height / 2 - stageRect.top }; const dx = target.x - start.x; const dy = Math.min(-24, target.y - start.y); const distance = Math.hypot(dx, dy); state.aim = { start, target, distance }; const percent = Math.round(clamp(distance / Math.max(90, stage.clientHeight * .38), .08, 1) * 100); powerValue.textContent = `${percent}%`; meterFill.style.width = `${percent}%`; guide.style.left = `${start.x}px`; guide.style.top = `${start.y}px`; guide.style.width = `${Math.max(52, Math.min(160, distance))}px`; guide.style.transform = `rotate(${Math.atan2(dy, dx) * 180 / Math.PI}deg)`; };
  ...
  const launch = () => { if (!state.aim) return; const { start, target, distance } = state.aim; const time = clamp(distance / (config.velocity * 300), .22, .58); state.ball = { x: start.x, y: start.y, vx: (target.x - start.x) / time, vy: (target.y - start.y - .5 * config.gravity * time * time) / time }; state.attempt += 1; state.phase = 'flying'; guide.style.opacity = '0'; updateStats(); };
  stage.addEventListener('pointerdown', (event) => { if (state.phase !== 'ready') return; event.preventDefault(); stage.setPointerCapture?.(event.pointerId); state.phase = 'aiming'; setAim(point(event)); });
  ...
  stage.addEventListener('pointercancel', () => { if (state.phase === 'aiming') state.phase = 'ready'; });
